...
def sauvegarder(df: pd.DataFrame, compteurs: dict):
    ...
    df_save.to_csv("hdfs_sequences.csv", index=False)
    taille = os.path.getsize("hdfs_sequences.csv") / (1024 * 1024)
    ...
    pd.DataFrame(rows).to_csv("hdfs_templates.csv", index=False)
...
if __name__ == "__main__":
    ...
    CHEMIN_LOG    = "HDFS.log"
    CHEMIN_LABELS = "anomaly_label.csv"
