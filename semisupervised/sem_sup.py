...
=============================================================
  ...
  DeepLog + MLP Comptage — VERSION CORRIGÉE
=============================================================

  CORRECTIONS APPLIQUÉES PAR RAPPORT À LA VERSION PRÉCÉDENTE
  ────────────────────────────────────────────────────────────

  CORRECTION 1 — Data Leakage (problème critique)
  ─────────────────────────────────────────────────
  AVANT : les 16 838 anomalies étaient utilisées à la fois
          pour entraîner le MLP ET dans le jeu de test.
          → Le MLP mémorisait les anomalies → F1=99.98% artificiel.

  APRÈS : les 16 838 anomalies sont divisées en deux parties
          strictement disjointes :
            70% (11 787) → entraînement du MLP uniquement
            30%  (5 051) → test uniquement, jamais vus par le MLP

  CORRECTION 2 — top_k non justifié
  ────────────────────────────────────
  AVANT : top_k=3, valeur arbitraire qui dégradait le LSTM.
  APRÈS : top_k=9, valeur du papier original DeepLog (2017),
          qui permet une comparaison directe avec la littérature.
          Une analyse de sensibilité est effectuée à la fin.

  CORRECTION 3 — Validation et early stopping du MLP
  ────────────────────────────────────────────────────
  AVANT : 20 epochs fixes, pas de validation.
  APRÈS : split 80/20 sur les données MLP + early stopping
          sur la validation loss.

  CORRECTION 4 — num_workers sur Colab
  ──────────────────────────────────────
  AVANT : num_workers=2 (cause des deadlocks sur Colab).
  APRÈS : num_workers=0 (stable sur tous les environnements).

  RÉSULTATS ATTENDUS APRÈS CORRECTION
  ─────────────────────────────────────
  LSTM seul (top_k=9) : F1 ≈ 0.89–0.94 (conforme DeepLog)
  MLP seul            : F1 honnête sur anomalies jamais vues
  Combiné OR          : Recall élevé, F1 compétitif
"""
...
CONFIG = {
    # Chemins
    "chemin_sequences" : "/content/drive/MyDrive/DeepLog3.0/semi-supervisé/hdfs_sequences.csv",
    "chemin_modele"    : "/content/drive/MyDrive/DeepLog3.0/semi-supervisé/deeplog_FINAL.pt",
