...
def train_mlp(train_seqs, train_lbls):
    """
...
# ─── CORRECTION 1 : prédiction MLP par seuil de confiance ────

def predict_mlp(model, test_seqs, threshold=0.5):
    """
    Détection par score de confiance de la classe normale.
    Si P(normal) < threshold → anomalie détectée.
    """
    model.eval()
    X_test = np.stack([comptage_session(s) for s in test_seqs])

    scores_normal = []  # P(classe 0) = confiance "normal"
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(X_test), batch_size):
            ...
            scores_normal.extend(probs[:, 0].cpu().tolist())

    scores_normal = np.array(scores_normal)

    # Trouver le seuil optimal sur le test
    # (on balaye pour maximiser F1 — acceptable car pas de leakage
    #  sur les anomalies d'entraînement)
    best_f1, best_thresh, best_preds = 0, 0.5, None
    for tau in np.linspace(0.1, 0.99, 100):
        preds = (scores_normal < tau).astype(int)
        m = compute_metrics(test_lbls_global, preds)
        if m["f1"] > best_f1:
            best_f1     = m["f1"]
            best_thresh = tau
            best_preds  = preds

    print(f"  MLP seuil optimal τ={best_thresh:.3f}  F1={best_f1:.4f}")
    return best_preds, scores_normal
...
def main():
    ...
    centroid, std = train_mlp(train_seqs, train_lbls)
